...
export const meta: CommandMeta = {
  ...
  version: "1.0.0",
  description: "Generate an image using the SDXL model based on a prompt.",
...
const API_URL = "http://65.109.80.126:20511/api/sdxl";
...
export const entry = defineEntry(
  async ({ input, output, args, langParser }) => {
    ...
    try {
      ...
      const filePath = path.join(CACHE_DIR, `${Date.now()}_sdxl_image.png`);
      ...
      console.error("SDXL Command Error:", err.message || err);
      await output.unsend(waitMsg.messageID);
