...
export const meta: CommandMeta = {
  ...
  version: "1.0.0",
  description: "Generate ultra-realistic AI images using Fluxx APIs",
  category: "AI",
...
export const langs = {
  en: {
    ...
    generating: "⚡ Fluxx is generating your images...",
...
const GEN_API = "http://65.109.80.126:20511/api/deto";
...
export const entry = defineEntry(
  async ({ input, output, args, langParser }) => {
    ...
    try {
      ...
      if (!data.status || !data.images || data.images.length !== 4) {
      ...
      // Combiner les images en une grille 2x2
      ...
      const outputPath = path.join(CACHE_DIR, `fluxx_${Date.now()}.png`);
...
export async function reply({
  ...
  const stream = await global.utils.getStreamFromURL(url, `fluxx_selected_${choice}.png`);

  output.reply({
    body: `✅ Here is your selected image (${choice}) from Fluxx.`,
    attachment: stream,
  });
}
